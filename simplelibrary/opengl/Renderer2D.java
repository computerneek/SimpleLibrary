...
public abstract class Renderer2D{
    ...
    public static void setTargetScreenSize(int width, int height){
        Renderer2D.width = width;
        Renderer2D.height = height;
    }
    public static void findScale(){
        widthScale = width/((double)Display.getWidth());
        heightScale = height/((double)Display.getHeight());
    }
    ...
    public static boolean isScaledClickWithinBounds(double clickX, double clickY, double targetXMin, double targetYMin, double targetXMax, double targetYMax){
        double scaledX = clickX*widthScale, scaledY = clickY*heightScale;
    ...
    public static boolean drawScaledText(double leftEdge, double topEdge, double rightPossibleEdge, double bottomEdge, String text){
        boolean trimmed = false;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftEdge&&!text.isEmpty()){
            trimmed = true;
        ...
        text.getChars(0, chars.length, chars, 0);
        for(char character : chars){
            drawScaledRect(leftEdge+skip, topEdge, leftEdge+skip+(FontManager.getCharLength(character)*scale), bottomEdge, FontManager.getFontImage(), FontManager.getTextureLocationForChar(character));
            skip+=(FontManager.getCharLength(character)*scale);
        }
        return true;
    }
    public static boolean drawScaledTextWithBounds(double leftEdge, double topEdge, double rightPossibleEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, String text){
        boolean trimmed = false;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftEdge&&!text.isEmpty()){
            trimmed = true;
        ...
        text.getChars(0, chars.length, chars, 0);
        for(char character : chars){
            drawScaledRectWithBounds(leftEdge+skip, topEdge, leftEdge+skip+(FontManager.getCharLength(character)*scale), bottomEdge, minX, minY, maxX, maxY, FontManager.getFontImage(), FontManager.getTextureLocationForChar(character));
            skip+=(FontManager.getCharLength(character)*scale);
        }
        return true;
    }
    public static String drawScaledTextWithWrap(double leftEdge, double topEdge, double rightPossibleEdge, double bottomEdge, String text){
        String originalText = text;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftEdge&&!text.isEmpty()){
            text = text.substring(0, text.length()-1);
    ...
    public static String drawScaledTextWithWrapAndBounds(double leftEdge, double topEdge, double rightPossibleEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, String text){
        String originalText = text;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftEdge&&!text.isEmpty()){
            text = text.substring(0, text.length()-1);
    ...
    public static boolean drawCenteredScaledText(double leftPossibleEdge, double topEdge, double rightPossibleEdge, double bottomEdge, String text){
        boolean trimmed = false;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>Math.abs(rightPossibleEdge-leftPossibleEdge)&&!text.isEmpty()){
            trimmed = true;
        ...
        text.getChars(0, chars.length, chars, 0);
        for(char character : chars){
            drawScaledRect(leftPossibleEdge+skip, topEdge, leftPossibleEdge+skip+(FontManager.getCharLength(character)*scale), bottomEdge, FontManager.getFontImage(), FontManager.getTextureLocationForChar(character));
            skip+=(FontManager.getCharLength(character)*scale);
        }
        return true;
    }
    public static boolean drawCenteredScaledTextWithBounds(double leftPossibleEdge, double topEdge, double rightPossibleEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, String text){
        boolean trimmed = false;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftPossibleEdge&&!text.isEmpty()){
            trimmed = true;
        ...
        text.getChars(0, chars.length, chars, 0);
        for(char character : chars){
            drawScaledRectWithBounds(leftPossibleEdge+skip, topEdge, leftPossibleEdge+skip+(FontManager.getCharLength(character)*scale), bottomEdge, minX, minY, maxX, maxY, FontManager.getFontImage(), FontManager.getTextureLocationForChar(character));
            skip+=(FontManager.getCharLength(character)*scale);
        }
        return true;
    }
    public static String drawCenteredScaledTextWithWrap(double leftPossibleEdge, double topEdge, double rightPossibleEdge, double bottomEdge, String text){
        String originalText = text;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftPossibleEdge&&!text.isEmpty()){
            text = text.substring(0, text.length()-1);
    ...
    public static String drawCenteredScaledTextWithWrapAndBounds(double leftPossibleEdge, double topEdge, double rightPossibleEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, String text){
        String originalText = text;
        double distanceForText = FontManager.getLengthForStringWithHeight(text, bottomEdge-topEdge);
        while(distanceForText>rightPossibleEdge-leftPossibleEdge&&!text.isEmpty()){
            text = text.substring(0, text.length()-1);
    ...
    public static void drawScaledRect(double leftEdge, double topEdge, double rightEdge, double bottomEdge, int texture){
        drawScaledRect( leftEdge, topEdge, rightEdge, bottomEdge, texture, 0, 0, 1, 1);
    }
    public static void drawScaledRect(double leftEdge, double topEdge, double rightEdge, double bottomEdge, int texture, double[] texLocation){
        drawScaledRect(leftEdge, topEdge, rightEdge, bottomEdge, texture, texLocation[0], texLocation[1], texLocation[2], texLocation[3]);
    }
    public static void drawScaledRect(double leftEdge, double topEdge, double rightEdge, double bottomEdge, int texture, double texLeft, double texUp, double texRight, double texDown){
        drawRect(leftEdge/widthScale, topEdge/heightScale, rightEdge/widthScale, bottomEdge/heightScale, texture);
    }
    public static void drawScaledRectWithBounds(double leftEdge, double topEdge, double rightEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, int texture){
        drawScaledRectWithBounds( leftEdge, topEdge, rightEdge, bottomEdge, minX, minY, maxX, maxY, texture, 0, 0, 1, 1);
    }
    public static void drawScaledRectWithBounds(double leftEdge, double topEdge, double rightEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, int texture, double[] texLocation){
        drawScaledRectWithBounds(leftEdge, topEdge, rightEdge, bottomEdge, minX, minY, maxX, maxY, texture, texLocation[0], texLocation[1], texLocation[2], texLocation[3]);
    }
    public static void drawScaledRectWithBounds(double leftEdge, double topEdge, double rightEdge, double bottomEdge, double minX, double minY, double maxX, double maxY, int texture, double texLeft, double texUp, double texRight, double texDown){
        drawRectWithBounds(leftEdge/widthScale, topEdge/heightScale, rightEdge/widthScale, bottomEdge/heightScale, minX/widthScale, minY/heightScale, maxX/widthScale, maxY/heightScale, texture, texLeft, texUp, texRight, texDown);
    }
    /**
